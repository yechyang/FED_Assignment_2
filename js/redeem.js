document.addEventListener("DOMContentLoaded", function () {
  ...
  function getVoucher(limit = 20, all = true) {
    ...
    fetch("https://fedassg-a6f6.restdb.io/rest/voucher", settings)
      ...
      .then(response => {
        ...
        for (var i = 0; i < response.length && i < limit; i++) {
          let item = response[i];
          let imagePath = '/FED_Assignment_2/images/' + encodeURIComponent(item.Voucher) + '.png';
          console.log(imagePath);

          content += `<div class="col-md-4">
            <div class="box text-center"> 
              <img src="${imagePath}" alt="${item.Voucher}">
              <p>${item.Voucher}</p>
